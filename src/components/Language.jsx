...
const Language = () => {
  ...
  return (
    <header>
      ...
      >
        <button className='text-2xl hover:text-yellow'>
          <LuLanguages />
        </button>
        {showMenu && (
          <div className='absolute top-full mt-2 right-0 w-40 bg-white bg-opacity-40 backdrop-blur-sm border border-grey shadow-lg rounded-md z-50'>
            ...
            <ul className='text-black text-lg'>
              {language === 'jp' ? (
                <>
                  <li
                    className='p-2 text-2xl hover:text-yellow cursor-pointer'
                    onClick={() => dispatch(switchToVietnamese())}
                  >
                    Vietnamese
                  </li>
                  <li
                    className='p-2 text-2xl text-yellow hover:text-yellow cursor-pointer'
